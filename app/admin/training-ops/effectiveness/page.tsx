...
export default function TrainingOpsEffectivenessPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                setLoading(true)
                const response = await ApiClient.getTrainingOpsEffectiveness()
            ...
            <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold">SME Effectiveness</h1>
                            <p className="mt-1 text-muted-foreground">
                                Track domain pass-rate movement against baseline, target, and challenge thresholds.
                            </p>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-3">
                        <Link href="/admin/training-ops/domains">
                            <Button variant="outline">Product Domains</Button>
                        </Link>
                        <Link href="/admin/training-ops/series">
                            <Button variant="outline">Learning Series</Button>
                        </Link>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-3xl">{loading ? '...' : summary.activeDomains}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-sm text-muted-foreground">Domains with live effectiveness data.</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2">
                            <CardDescription>Average Current Pass Rate</CardDescription>
                            <CardTitle className="text-3xl">{loading ? '...' : `${summary.avgPassRate}%`}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-sm text-muted-foreground">Mean pass rate across the filtered domains.</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2">
                            <CardDescription>On Track</CardDescription>
                            <CardTitle className="text-3xl">{loading ? '...' : summary.onTrack}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-sm text-muted-foreground">Domains currently meeting target pass rate.</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2">
                            <CardDescription>At Risk</CardDescription>
                            <CardTitle className="text-3xl">{loading ? '...' : summary.atRisk}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-sm text-muted-foreground">Domains already below their challenge threshold.</p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Filter by domain name, SME owner, category, or track.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Input
                            value={search}
                            onChange={(event) => setSearch(event.target.value)}
                            placeholder="Search domains or SME owners..."
                        />
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Current pass rate is calculated from graded attempts on exams mapped to each product domain.
                        </CardDescription>
                    </CardHeader>
                                    ...
                                    <div className="flex flex-wrap items-start justify-between gap-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <Badge variant="outline">
                                                {row.linkedExamCount} exams
                                            </Badge>
                                            <Badge variant="outline">
                                                {row.scheduledEventCount} scheduled events
                                            </Badge>
                                            <Badge variant="outline">
                                                {row.gradedAttempts} graded attempts
                                            </Badge>
                                        </div>
                                    </div>
