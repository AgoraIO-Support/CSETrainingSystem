...
import type { TrainingOpsLearningSeriesImportSummary } from '@/types'

export default function ImportTrainingOpsSeriesPage() {
    const [rawJson, setRawJson] = useState('')
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)
    const [summary, setSummary] = useState<TrainingOpsLearningSeriesImportSummary | null>(null)
    ...
    const submitImport = async (apply: boolean) => {
        ...
        try {
            ...
            const response = await ApiClient.importTrainingOpsLearningSeries({
                payload,
                apply,
            })
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                ...
                                placeholder='Paste the contents of prisma/seed-data/training-ops-learning-series.v1.json here...'
                                ...
                                {summary.items.map((item) => (
                                    <div key={item.slug} className="rounded-lg border p-4">
                                        ...
                                        <p className="mt-2 text-sm text-muted-foreground">
                                            Domain: {item.domainSlug || 'n/a'} · Owner: {item.ownerEmail || 'n/a'}
                                        </p>
                                    </div>
                                ))}
                            </div>
