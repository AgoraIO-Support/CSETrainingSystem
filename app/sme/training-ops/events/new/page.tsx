...
function NewSmeLearningEventPageContent() {
    ...
    useEffect(() => {
        const loadOptions = async () => {
            try {
                const [overviewRes, hostRes] = await Promise.all([
                ...
                if (queryDomainId || querySeriesId) {
                    const matchedSeries = querySeriesId
                        ? overviewRes.data.series.find((item) => item.id === querySeriesId)
                        : null
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await ApiClient.createSmeTrainingOpsEvent({
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Define the session title, format, ownership, and scheduling metadata.</CardDescription>
                        </CardHeader>
