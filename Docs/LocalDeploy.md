  ...
  7)（可选）启动 Fastify backend
  只有当你要用到「materials CloudFront cookie」或「部分 delete 代理接口」才需要；并且它要求 CloudFront signing 配置（本地一般可以先不跑）。

  8) 常见问题排查

  - 上传文件时报 CORS：检查 S3 CORS（AllowedOrigins/Headers/Methods）。
  - presigned PUT 403：检查 AWS_REGION 是否与 bucket 一致、IAM 权限、以及 env 是否正确传进容器。
  - DB 连不上：确认容器都在 cselearning network；DATABASE_URL host 用 cselearning-postgres（不是 localhost）。

  A) 什么时候需要 backend

  - 需要：课程 materials 的 CloudFront signed-cookie（/api/materials/.../cf-cookie）和部分 Admin 删除操作（Next.js 会代理到 backend）。
  - 不需要：普通课程/章节/课时创建、上传 MP4/VTT（这些走 Next.js + S3 presigned）。

  B) 准备 backend 环境变量
  backend 启动时强制要求 CloudFront 相关变量（否则会直接报错退出），建议建一个文件 tmp/podman/backend.env：

  cat > tmp/podman/backend.env <<'EOF'
  PORT=8080
  ...
  - 看日志：podman logs -f cselearning-backend
  - 在后台页面执行一次“删除课程/章节/课时”等需要代理到 backend 的操作，能成功就说明链路 OK。
