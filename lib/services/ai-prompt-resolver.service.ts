...
function getFallbackChatModel(provider: LLMProviderId, openAIModel = 'gpt-4o-mini', vexkeModel = 'gpt-5.4'): string {
    return provider === 'vexke' ? vexkeModel : openAIModel
}

const LESSON_COACH_SYSTEM_PROMPT = [
...
const LESSON_COACH_USER_PROMPT = [
    'Course: {{courseTitle}}',
...
- No markdown, no extra keys, no trailing comments.`,
        ...
        case AIPromptUseCase.LEARNING_AGENT_LESSON_COACH:
            return {
                source: 'fallback',
                systemPrompt: LESSON_COACH_SYSTEM_PROMPT,
                userPrompt: LESSON_COACH_USER_PROMPT,
                provider,
                model: getFallbackChatModel(provider),
                temperature: 0.2,
                maxTokens: 1600,
                responseFormat: AIResponseFormat.TEXT,
            }
        case AIPromptUseCase.LEARNING_AGENT_EXAM_MISTAKE_REVIEW:
            return {
                source: 'fallback',
                systemPrompt: 'Learning Agent Exam Mistake Review runtime prompt is generated by the application.',
                userPrompt: null,
                provider,
                model: getFallbackChatModel(provider),
                temperature: 0.2,
        ...
        case AIPromptUseCase.LEARNING_AGENT_PLAN:
            return {
                source: 'fallback',
                systemPrompt: 'Learning Agent Learning Plan runtime prompt is generated by the application.',
                userPrompt: null,
                provider,
                model: getFallbackChatModel(provider),
                temperature: 0.2,
            ...
            if (templateId) {
                ...
                if (t?.isActive) {
                    return {
                        ...
                        provider: (t.provider as LLMProviderId) || 'openai',
                        model: t.model,
            ...
            if (courseId) {
                ...
                if (courseAssignment?.isEnabled && courseAssignment.template?.isActive) {
                    ...
                    return {
                        ...
                        systemPrompt: t.systemPrompt ?? t.template,
                        userPrompt: t.userPrompt ?? null,
                        provider: (courseAssignment.providerOverride as LLMProviderId | null) ?? (t.provider as LLMProviderId) ?? 'openai',
            ...
            if (examId) {
                ...
                if (examAssignment?.isEnabled && examAssignment.template?.isActive) {
                    ...
                    return {
                        ...
                        systemPrompt: t.systemPrompt ?? t.template,
                        userPrompt: t.userPrompt ?? null,
                        provider: (examAssignment.providerOverride as LLMProviderId | null) ?? (t.provider as LLMProviderId) ?? 'openai',
